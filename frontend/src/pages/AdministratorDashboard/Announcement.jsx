...
export default function AdminAnnouncements() {
    ...
    // fetchList (replace your existing fetchList mapping block)
    const fetchList = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(`http://localhost:8080/api/admin/announcements?${params.toString()}`, {
            ...
            // Robust normalization: prefer isActive/isPinned/content but fall back to active/pinned/body
            const normalized = (data.content || []).map(i => ({
                ...i,
                // prefer the explicit server fields but fallback if not present
    ...
    // openEdit (replace your existing openEdit)
    const openEdit = (ann) => {
        setEditing(ann.id);
        setForm({
            title: ann.title || '',
    ...
    const save = async () => {
        ...
        try {
            const payload = {
                title: form.title,
                content: form.body,
                visibility: form.visibility, // "ALL" | "OFFICERS"
    ...
    // toggleActive (replace your existing toggleActive)
    const toggleActive = async (ann) => {
        if (actionInProgress) return;
        setActionInProgress(true);

        // Compute current value robustly
        ...
        try {
            // Build payload matching AnnouncementCreateUpdateDTO expected by PUT /api/admin/announcements/{id}
            const payload = {
                title: ann.title || '',
            ...
            const res = await fetch(`http://localhost:8080/api/admin/announcements/${ann.id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    const remove = async (ann) => {
        ...
        try {
            ...
            // Optionally re-sync to ensure server and UI are consistent
            await fetchList();
        } catch (e) {
            console.error(e);
    ...
    return (
        <div className="admin-announcements">
            <div className="header">
                <h1>Announcements</h1>
                <button className="primary" onClick={openCreate} disabled={loading}>New Announcement</button>
            </div>
            ...
            <div className="list">
                ...
                {loading ? <div className="loading">Loading...</div> : (
                    <table className="table">
                        ...
                        {items.length === 0 ? (
                            <tr>
                                <td colSpan="8" style={{textAlign: 'center', padding: '1.5rem'}}>No announcements</td>
                            </tr>
                        ) : items.map(a => (
                            <tr key={a.id}>
                                <td>{a.title}</td>
                                ...
                                <td><span
                                    className={`badge ${a.active ? 'green' : 'gray'}`}>{a.active ? 'Active' : 'Inactive'}</span>
                                </td>
                                ...
                                <td className="actions">
                                    ...
                                    <button
                                        onClick={() => toggleActive(a)} disabled={actionInProgress}>{a.active ? 'Deactivate' : 'Activate'}</button>
                    ...
                    <div className="modal" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="modal-body">
                            ...
                            <div className="row">
                                ...
                                <label><input type="checkbox" checked={form.pinned}
                                              onChange={e => setForm(f => ({...f, pinned: e.target.checked}))}/> Pinned
                                    (optional)</label>
                            </div>
